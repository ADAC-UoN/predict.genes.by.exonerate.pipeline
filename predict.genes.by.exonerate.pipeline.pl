...
$time = scalar localtime;
print "$time Finished Finding non-overlapping predicted genes\n";
...
$time = scalar localtime;
print "$time Create Fasta file of predicted genes\n";
...
open KEEPFASTAOUT, ">$prefix.ex.nonoverlapping.predictions.fasta";
while (<KEEPTABIN>){
        chomp $_;
        my $line = $_;
        if ($line =~ /(.*)\t(.*)/)
